...
contract AutomationTask is AutomationCompatible {
    ...
    constructor(uint256 _interval) {
        ...
        //在此添加 solidity 代码
        healthPoint = initHP(MAXIMUM_HEALTH);
    }

    function initHP(uint value) private pure returns (uint[SIZE] memory) {
        uint256[SIZE] memory hp_arr;
    ...
    function fight(uint256 fighter) public {
        ...
        healthPoint = initHP(healthPoint[0] - fighter);
    }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        //在此添加和修改 solidity 代码
        if (
            healthPoint[0] < MAXIMUM_HEALTH &&
            interval <= block.timestamp - lastTimeStamp
        ) {
            upkeepNeeded = true;
            performData = abi.encode(true); // 存储 true 表示需要维护
        } else {
            performData = abi.encode(false); // 存储 false 表示不需要维护
        }
    ...
    function performUpkeep(bytes memory performData) external override {
        //在此添加 solidity 代码
        bool needsUpkeep = abi.decode(performData, (bool));

        // 在此执行实际维护操作，根据 performData 的值决定是否进行维护
        if (needsUpkeep) {
            // 执行维护操作
            healthPoint = initHP(MAXIMUM_HEALTH);
        }
